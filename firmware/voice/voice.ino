/*
   ...
   `---'.|                ---`-'       :  \  \ :  ,      .-./ `--''           '---'        |  ,     .-./  ---`-'               |   :    :
    ...
    v4.4
    - Use PIO for control DCO frequency
    - Use dma for control DCO rampe amplitude 
    - Use dma for the DSO with oversamplig x16 and linear interpolation
    - Fix PWM_B bug
    - Fix DSO Bug
...
void setup() {
  ...
  as.set_DcoA_freq(220);
  as.set_DcoB_freq(222);
  ...
  dso.setAmplitude(120);
  dso.setFrequency(220);
  dso.setWaveform(3);
