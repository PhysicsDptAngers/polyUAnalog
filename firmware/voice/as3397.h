...
const float MAXFREQ = 12544;
const float _div = 2;

const int CAPTURE_DEPTH = 100;
const float Fe = 1000000;

static int CAPTURE_CHANNEL = 0;
static uint16_t capture_buf[CAPTURE_DEPTH];
static uint dma_chan_adc;
...
struct CallbackData {
  ...
  long error = 0, output = 0;                          //Variables de la commande PI
  long errorSum[128] = {};
  uint8_t note;
#if PIDFIXE == 0
  float _Kp = 440 * Kp;
...
public:
  ...
  void set_DcoA_freq(float freq);
  void set_DcoB_freq(float freq);
  void set_DcoAFM(float FMmod);
  void set_DcoBFM(float FMmod);
  ...
  uint32_t getSliceDsoMSB() {
    return Dso.sliceH;
  };
  uint32_t getSliceDsoLSB() {
    return Dso.sliceL;
  };
  uint32_t getChannelDsoMSB() {
    return Dso.channelH;
  };
  uint32_t getChannelDsoLSB() {
    return Dso.channelL;
  };
};
